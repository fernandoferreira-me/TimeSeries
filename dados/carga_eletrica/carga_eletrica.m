...
%Seleciona as séries a serem analisadas
%serie_alvo: linha da matriz contendo a serie alvo. Se zero, utiliza-se o valor máximo
%limiar_corr: valor da correlação para selecionar a série explicativa (se zero, utiliza-se a correlação com a temperatura)
...
switch periodo
    case 1
    %Serie teste é janeiro
    ajuste_inicial=16;%Numero de amostras necessárias para se compor o modelo e não precisar estimar as condições iniciais
...
switch flag
    ...
    if limiar_corr ==0
        c=corrcoef([serie_trval_completa;temperatura_trval]');limiar_corr=abs(c2(1,49));%para o caso de seleção pelo nivel de correlacao com a temperatura
    end
