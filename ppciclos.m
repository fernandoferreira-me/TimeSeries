function [saida, model] = ppciclos(serie)
%Permite a avaliação da presença de ciclos (visual) e o pre-processamento(fourier) da série
%Ciclos%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
n1=size(serie,1);
retirada=cell(n1,1);
residuo=cell(n1,1);
...
grau(i)=input('Digite o grau do polinômio a extrair: ');%0 não extrai polinomio
nc(i)=input('Digite o número de componentes a retirar: ');%0 não extrai ciclo
...
model{1}=grau;
model{2}=nc;
model{3}=retirada;%Modelo pp ciclos
